...
- **🔊 / ⏬ ボタン**でその文を読み上げる／その文から記事の最後まで続けて読みます。
...
記事は文単位でも、通しでも読み上げられます。**言語は文ごとに自動で切り替わる**ので、
「通して聞く」で号全体を再生すると、英語 → 中国語 → スペイン語 → フランス語と声が入れ替わります。

| 操作 | 場所 | はたらき |
|---|---|---|
| 🔊 | 各文 | その文を読む（リピート設定ぶん繰り返す） |
| ⏬ | 各文 | **その文から記事の最後まで**続けて読む |
| ▶ この記事を通して聞く | 記事の見出し | 記事1本を最初から通して読む |
| 🎧 通して聞く | ツールバー | いま表示中の記事すべてを通して読む |
| ⚙︎ 音声設定 | ツールバー | 速度・リピート・文の間・言語ごとの声 |

再生中は画面下にプレーヤーが出ます。

```
⏮  ⏸  ⏭   🇬🇧 ENGLISH  3 / 10  [2/3回目]   速度 0.9×   ■
```

- **⏮ / ⏭** で前後の文へ。**⏸** で一時停止、**■** で停止
- 読んでいる文が枠で示され、画面外なら自動でスクロールします
- キーボード：`k` で一時停止／再開、`Esc` で停止

### シャドーイングに使う

音声設定で **リピートを2〜3回**、**文の間を2〜3秒**、**速度を0.7×前後**にすると、
「1文を複数回読む → 間があく → 次の文へ」という練習用の再生になります。
`LISTENING & SHADOWING` の各文にも再生ボタンがあります。

### 声について

読み上げには**端末にインストールされている音声**を使います（ブラウザの Web Speech API）。
音声設定パネルで言語ごとに声を選べます。入っていない言語は「この端末に音声が入っていません」と表示され、
その言語だけ再生されません。OSの音声設定から追加できます。

- macOS：システム設定 → アクセシビリティ → 読み上げコンテンツ → システムの声 → 声を管理
- Windows：設定 → 時刻と言語 → 音声認識 → 音声の追加
- iOS / Android：設定内の「読み上げ」「テキスト読み上げ」から言語パックを追加

音声合成に対応していないブラウザでは、読み上げ関連のボタンは自動的に隠れ、
それ以外の機能は通常どおり使えます。
